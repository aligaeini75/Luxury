...
womanRoutes.post('/availability', async c => {
  ...
  await c.env.DB.prepare(`INSERT INTO woman_availability(
    ...
    .bind(
      id('av'),u.id,type,String(b.date||''),String(b.start_time||''),String(b.end_time||''),
      Number(b.duration_minutes||60),price,0,
      String(b.location_title||''),String(b.location_hint||''),JSON.stringify(photos),JSON.stringify(services),price,'active',t,t
    ).run()
...
womanRoutes.patch('/availability/:id', async c => {
  ...
  await c.env.DB.prepare(`UPDATE woman_availability SET
    ...
    .bind(
      type,String(b.date||current.date||''),String(b.start_time||current.start_time||''),String(b.end_time||current.end_time||''),
      Number(b.duration_minutes||current.duration_minutes||60),price,
      String(b.location_title ?? current.location_title ?? ''),String(b.location_hint ?? current.location_hint ?? ''),
      JSON.stringify(photos),JSON.stringify(services),price,t,avId,u.id
    ).run()
