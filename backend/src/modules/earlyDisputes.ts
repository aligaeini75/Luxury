...
async function getAccess(c: any, booking: any) {
  ...
  const ids = [booking.man_id, booking.woman_id, booking.user_id, booking.requester_id, booking.customer_id]
    .filter(Boolean)
    .map((x: any) => String(x))

  if (uid && ids.includes(uid)) return true
  if (email && [booking.man_email, booking.woman_email].map((x: any) => String(x || '').toLowerCase()).includes(email)) return true
  return false
}
...
function bookingAccessPayload(booking: any) {
  ...
  return {
    allowed: status === 'accepted' && (early || (communication && byTime)),
    status,
    booking_type: type,
...
async function insertAlert(c: any, userId: string, title: string, body: string, targetUrl = '/#/man/requests') {
  if (!userId) return
  await c.env.DB.prepare(`
    INSERT INTO user_alerts(id,user_id,type,title,body,target_url,is_read,created_at)
    VALUES(?,?,?,?,?,?,0,datetime('now'))
  `).bind(crypto.randomUUID(), userId, 'booking_admin_action', title, body, targetUrl).run().catch(() => {})
}

async function listDisputes(c: any) {
  return await c.env.DB.prepare(`
...
async function listEarly(c: any) {
  return await c.env.DB.prepare(`
...
earlyDisputesRoutes.get('/bookings/:id/access', async c => {
  const booking: any = await getBooking(c, c.req.param('id'))
...
earlyDisputesRoutes.post('/bookings/:id/early-open/request', async c => {
  ...
  await c.env.DB.prepare(`INSERT INTO booking_early_open_requests(id,booking_id,requester_id,note,status,created_at) VALUES(?,?,?,?, 'pending', datetime('now'))`)
    .bind(id, bookingId, currentUserId(c) || null, body.note || '').run()

  const targetUserId = otherSideUserId(c, booking)
  await insertAlert(c, targetUserId, 'درخواست باز شدن زودتر', 'طرف مقابل درخواست داده چت یا ویدیوکال زودتر باز شود.', currentUser(c).role === 'woman' ? '/#/man/requests' : '/#/woman/requests')
...
earlyDisputesRoutes.post('/bookings/:id/disputes', async c => {
  ...
  const id = crypto.randomUUID()
  await c.env.DB.prepare(`INSERT INTO booking_disputes(id,booking_id,reporter_id,reason,status,created_at) VALUES(?,?,?,?, 'open', datetime('now'))`)
...
earlyDisputesRoutes.get('/admin/early-disputes', async c => {
  const disputes = await listDisputes(c)
  const early = await listEarly(c)
...
earlyDisputesRoutes.post('/admin/early-open/:id/:action', async c => {
  ...
  await c.env.DB.prepare(`UPDATE booking_early_open_requests SET status=?, admin_note=?, decided_at=datetime('now') WHERE id=?`).bind(status, body.note || '', id).run()
  ...
  for (const uid of [booking.man_id, booking.woman_id].filter(Boolean)) {
    await insertAlert(c, uid, status === 'approved' ? 'باز شدن زودتر تایید شد' : 'درخواست باز شدن زودتر رد شد', status === 'approved' ? 'ادمین تایید کرد؛ ارتباط برای هر دو طرف باز شد.' : 'ادمین درخواست باز شدن زودتر را رد کرد.')
  }
...
earlyDisputesRoutes.post('/admin/disputes/:id/resolve', async c => {
  ...
  await c.env.DB.prepare(`UPDATE booking_disputes SET status=?, admin_note=?, resolved_at=datetime('now') WHERE id=?`).bind(status, body.note || '', id).run()
  ...
  if (booking) {
    for (const uid of [booking.man_id, booking.woman_id].filter(Boolean)) {
      await insertAlert(c, uid, 'اختلاف بررسی شد', `وضعیت بررسی ادمین: ${status}`)
    }
  }
