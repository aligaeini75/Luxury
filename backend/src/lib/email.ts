...
function html(env: EmailEnv, o: EmailOptions) {
  ...
  return `<!doctype html><html lang="fa" dir="rtl"><body style="margin:0;background:#020100;color:#fff;font-family:Tahoma,Arial,sans-serif;direction:rtl;text-align:right"><table width="100%" cellpadding="0" cellspacing="0" style="padding:34px 12px;background:radial-gradient(circle at 85% 0%,rgba(214,173,82,.28),transparent 34%),linear-gradient(180deg,#020100,#080502)"><tr><td align="center"><table width="100%" cellpadding="0" cellspacing="0" style="max-width:680px;border:1px solid rgba(214,173,82,.28);border-radius:34px;overflow:hidden;background:linear-gradient(145deg,rgba(255,255,255,.075),rgba(255,255,255,.018));box-shadow:0 30px 110px rgba(214,173,82,.16)"><tr><td style="padding:34px;background:linear-gradient(135deg,rgba(214,173,82,.34),rgba(0,0,0,.84))"><div style="display:inline-block;border:1px solid rgba(255,240,189,.28);border-radius:999px;padding:9px 14px;color:#fff0bd;font-size:12px;font-weight:900">LUXORA PRIVATE ACCESS</div><h1 style="margin:24px 0 0;color:#fff0bd;font-size:32px;line-height:1.3">${esc(o.title)}</h1></td></tr><tr><td style="padding:34px"><p style="margin:0;color:#d8c99d;font-size:17px;line-height:2">${esc(o.body).replace(/\n/g, '<br>')}</p>${code}${btn}<div style="margin-top:30px;padding-top:18px;border-top:1px solid rgba(255,255,255,.08);color:#8f805f;font-size:12px">این پیام به صورت خودکار از Luxora ارسال شده است.</div></td></tr></table></td></tr></table></body></html>`
}

export async function sendLuxEmail(env: EmailEnv, o: EmailOptions) {
  if (!o.to) return { ok: false, skipped: true }
  ...
  const res = await fetch('https://api.resend.com/emails', {
    ...
    body: JSON.stringify({ from: env.EMAIL_FROM || 'Luxora <noreply@luxora.app>', to: [o.to], subject: o.subject, html: html(env, o) })
  })
